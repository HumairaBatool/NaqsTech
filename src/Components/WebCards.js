    ...
    render() {
      ...
      return (
            <div>
                ...
                <div className="flex justify-evenly grid-cols-4 space-x-1 text-center ml-64 mr-64 h-60 mt-28 mb-8 ">
                    {cardData.map((card) => (
                    <div key={card.id} className={`w-56 bg-${card.bgColor} bg-[#F48029] rounded-lg`}>
                        <img className=" ml-20 mt-16" src={card.imageUrl} alt={` ${card.title}`} />
                        <h1 className="text-white text-[20px] mt-6">{card.title}</h1>
                    </div>
                    ))}
                </div>
                <div className="flex justify-evenly space-x-1 grid-cols-4 text-center ml-64 mr-64 h-60 mb-10">
                    {cardData2.map((card) => (
                    <div key={card.id} className={`w-56 bg-${card.bgColor} bg-[#F48029] rounded-lg`}>
                        <img  className=" ml-20 mt-16" src={card.imageUrl} alt={` ${card.title}`} />
